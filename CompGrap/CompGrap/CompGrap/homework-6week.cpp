...
bool AABBOverlap(Vec2 minA, Vec2 maxA, Vec2 minB, Vec2 maxB)
{
    return (minA.x <= maxB.x && maxA.x >= minB.x) &&
        (minA.y <= maxB.y && maxA.y >= minB.y);
}

Vec2 ComputeAABBCenter(Vec2 min, Vec2 max)
{
    return { (min.x + max.x) / 2.0, (min.y + max.y) / 2.0 };
}

void FindIntersections(BVH* bvhA, BVH* bvhB)
{
    if (bvhA == nullptr || bvhB == nullptr)
        return;

    std::queue<BV*> queueA, queueB;
    queueA.push(&bvhA->roots[0]);
    queueB.push(&bvhB->roots[0]);

    while (!queueA.empty() && !queueB.empty())
    {
        auto bvA = queueA.front();
        queueA.pop();

        auto bvB = queueB.front();
        queueB.pop();

        if (AABBOverlap(bvA->min, bvA->max, bvB->min, bvB->max))
        {
            if (bvA->left_ == nullptr && bvA->right_ == nullptr &&
                bvB->left_ == nullptr && bvB->right_ == nullptr)
            {
                Vec2 centerA = ComputeAABBCenter(bvA->min, bvA->max);
                Vec2 centerB = ComputeAABBCenter(bvB->min, bvB->max);

                // TODO: Check if the centers are close enough to consider it an intersection
                // If yes, draw the intersection point
            }
            else
            {
                if (bvA->left_ != nullptr && bvB->left_ != nullptr)
                {
                    queueA.push(bvA->left_);
                    queueB.push(bvB->left_);
                }
                if (bvA->right_ != nullptr && bvB->right_ != nullptr)
                {
                    queueA.push(bvA->right_);
                    queueB.push(bvB->right_);
                }
            }
        }
    }
